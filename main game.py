...
def play(PLAYER_POS): #maybe pass in save_point as a parameter, and set it to True when you pass checkpoint i.e. play(PLAYER_POS,save_point) would be play(PLAYER_POS,True)
    ...
    scroll = [0,0] #how far screen scrolls by on x and y axis - for camera movement 
   ...
   '''
    ...
    while True: #making sure that all the changes remain on the screen and don't disappear straight away
        ...
        scroll[0] -= (PLAYER_POS[0]+scroll[0]-x/2+PLAYER.get_width()/2)/30 #how fast/slow the camera catches up to the player's central position (can be changed by the divider e.g. 30 -> 50 is slower)
        scroll[1] -= (PLAYER_POS[1]-y/2+PLAYER.get_height()/2+scroll[1])/30
        ...
        if melee == True:
            ...
            obba_center = (PLAYER_POS[0]+50 - (obba.get_width()/2) + scroll[0],PLAYER_POS[1]+80 - (obba.get_height()/2) + scroll[1])
            SCREEN.blit(obba,obba_center)
       
       
        if ranged == True:
            SCREEN.blit(shotgun.get_image(index),(shotgun.get_posx()+10 + scroll[0],shotgun.get_posy()+70 + scroll[1]))
        ...
        if enemy_x < PLAYER_POS[0] + scroll[0]:
        ...
        if enemy_x == PLAYER_POS[0] + scroll[0]:
            dandelion.set_posx(PLAYER_POS[0] + scroll[0])
        
        if enemy_y < PLAYER_POS[1]-70 + scroll[1]: #offsetting y pos of enemy so it doesn't look like it's through the floor
            dandelion.set_posy(enemy_y+enemy_y_speed)
    
        if enemy_y > PLAYER_POS[1]-70 + scroll[1]:
            dandelion.set_posy(enemy_y-enemy_y_speed)
        
        if enemy_y == PLAYER_POS[1] + scroll[1]:
            dandelion.set_posy(PLAYER_POS[1] + scroll[1])
        ...
        if swing == True:
            if melee == True:
                angle = angle-6 #turns through clockwise (-ve is clockwise, +ve anticlockwise)      
                if angle != -90:
                    obbabox = pygame.Rect(PLAYER_POS[0] + PLAYER.get_width()/2 + scroll[0] , PLAYER_POS[1] + PLAYER.get_height()/2 - HORIZON.get_height() + 20 + scroll[1], HORIZON.get_height(),HORIZON.get_height())
