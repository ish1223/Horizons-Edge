...
ally = Ally(200,600,venom_left,venom_right)
ally.set_images([wizard_cat_left,wizard_cat_right,wizard_cat_right])
ally.set_dmg(100)
...
def play(PLAYER_POS,PLAYER_hp): #maybe pass in save_point as a parameter, and set it to True when you pass checkpoint i.e. play(PLAYER_POS,save_point) would be play(PLAYER_POS,True)
    ...
    while True: #making sure that all the changes remain on the screen and don't disappear straight away
        ...
        for h in range(len(enemies)):
            ...
            if enemies[h].get_Rect().colliderect(FLOOR_right_rect) == False and enemies[h].get_Rect().colliderect(FLOOR_left_rect) == False:
        ...
        for h in range(len(enemies)):
            ...
            if enemies[h] != ally: #make it ao ally doesn't hurt you
        ...
        for h in range(len(enemies)):
            ...
            if abs(enemies[h].get_posx() - PLAYER_POS[0]) == min(distances): #finding enemy with closest to player so ally knows which enemy to target 
                target = enemies[h]
            ...
            if enemies[h] == ally:
                enemies[h].set_speed(20) #testing if ally can target closest enemy (works)
                #enemies[h].movex(target.get_posx(),scroll[0],moving_right,moving_left,distance/2)
                target.hp = enemies[h].atk(SCREEN,target.get_posx(),target.get_Rect(),target.get_hp(),scroll[0],scroll[1])
            else:
