...
def play(PLAYER_POS): #maybe pass in save_point as a parameter, and set it to True when you pass checkpoint i.e. play(PLAYER_POS,save_point) would be play(PLAYER_POS,True)
   ...
   '''
    ...
    while True: #making sure that all the changes remain on the screen and don't disappear straight away
        ...
        #0 -= (PLAYER_POS[0]+0-x/2+PLAYER.get_width()/2)/30 #how fast/slow the camera catches up to the player's central position (can be changed by the divider e.g. 30 -> 50 is slower)
        #0 -= (PLAYER_POS[1]-y/2+PLAYER.get_height()/2+0)/30
        ...
        #----------------------------------------------------
        #code for rotating image animation
        if melee == True:
        ...
        dandframe = dandelion.get_image(index)
        setdandrect = dandelion.set_Rect(index)
        dandrect = dandelion.get_Rect()
        
        if dandelion.get_hp() > 0:
            dandrect.center = (currentx + (dandframe.get_width()/2),currenty + (dandframe.get_height()/2))
        else:
            dandrect.center = (9000,9000)
        ...
        if swing == True:
            if melee == True:
                angle = angle-6 #turns through clockwise (-ve is clockwise, +ve anticlockwise)      
                if angle != -90:
                    obbabox = pygame.Rect(PLAYER_POS[0]+PLAYER.get_width()/2,PLAYER_POS[1]+PLAYER.get_height()/2-HORIZON.get_height()+20,HORIZON.get_height(),HORIZON.get_height())
        ...
        for event in pygame.event.get(): #this loop is used for all events e.g. button presses or mouse clicks
            ...
            if event.type == KEYUP:
                ...
                if event.key == K_w or event.key == K_UP: #switching weapoons
                    if ranged == True:
                        ranged = False
                        melee = True
                    elif melee == True:
